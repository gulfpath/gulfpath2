...
export default function Register() {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const mockToken = btoa(JSON.stringify({ role, phone, email, name, timestamp: Date.now() }));
                ...
                <form className="space-y-5" onSubmit={handleSendOTP}>
                  ...
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium text-slate-300 mb-1.5">
                      {role === "worker" ? "Full Name (As per Passport/Aadhaar)" : "Your Full Name"}
                    </label>
                    <input
                      type="text"
                      name="name"
                  ...
                  <button
                    type="submit"
                    disabled={isLoading || phone.length < 10 || name.length < 2 || (role === "employer" && (!email || companyName.length < 2))}
